<?php

return [
    'trips_retrieved' => 'تم استرجاع الرحلات بنجاح.',
    'tours_retrieved' => 'تم استرجاع الجولات بنجاح.',
    'tour_retrieved' => 'تم استرجاع الجوله بنجاح.',
    "trip_retrieved" => "تم استرجاع الرحلة بنجاح",
    "trip_tours_retrieved" => "تم استرجاع رحلات الجوله بنجاح",
    "trip_tour_retrieved" => "تم استرجاع رحله الجوله بنجاح",
    'trending_trips_retrieved' => 'تم استرجاع الرحلات المشهورة بنجاح.',
    'trip_not_found'  => 'الرحلة غير موجودة.',
    'success_operation' => 'تمت العملية بنجاح',
    'trending' => "مشهوره",
    'nottrending' => "غير مشهورة",
    'child_age_limit' => 'الحد الأقصى لسن الأطفال هو :age سنة',
    'too_many_requests' => 'لقد تجاوزت عدد المحاولات المسموح بها. برجاء المحاولة مرة أخرى بعد دقيقة.',
    "tour_availability_retrieved"=>"تم استرجاع الرحلات اليوميه بنجاح",
    'no_seats_available' => 'عفواً، لا يوجد أماكن كافية المتاحة حالياً.',
    'tour_not_available' => 'عفواً، هذه الجولة غير متاحة للحجز حالياً.',
];
